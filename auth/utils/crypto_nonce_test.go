...
func TestCreateNonce(t *testing.T) {
	...
	testWallet := wallet.NewWallet(privateKey)

	// Test creating a nonce
	nonce, err := CreateNonce(testWallet, wallet.CounterpartyTypeSelf)
	...
	nonce2, err := CreateNonce(testWallet, wallet.CounterpartyTypeSelf)
...
func TestVerifyNonce(t *testing.T) {
	...
	nonce, err := CreateNonce(testWallet, counterpartyType)
	assert.NoError(t, err, "Failed to create nonce")

	// Verify the valid nonce
	valid, err := VerifyNonce(nonce, testWallet, counterpartyType)
	...
	valid, err = VerifyNonce("invalidnonce", testWallet, counterpartyType)
	...
	valid, err = VerifyNonce(nonce, testWallet, wallet.CounterpartyTypeAnyone)
